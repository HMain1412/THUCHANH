...
import userModel from '../Models/userModel'  // Import model xử lý CSDL
...
const authMiddleware = (req, res, next) => {
    if (!req.session.user) {
        return res.redirect('/login'); //Hướng đến trang đăng nhập nếu chưa đăng nhập
    }
    next();
};
...
const adminMiddleware = (req, res, next) => {
    if (req.session.user.role !== 'admin') {
        return res.status(403).send('Access denied'); //Không phải là admin trả lỗi 403 
    }
    next();
};
...
const userMiddleware = async (req, res, next) => {
    ...
    try {
        ...
        // admin, có quyền thực hiện mọi thao tác
        if (sessionUser.role === 'admin') {
            return next();
        }

        //  user, chỉ cho phép thao tác trên tài khoản của chính mình
